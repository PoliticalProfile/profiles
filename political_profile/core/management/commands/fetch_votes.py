...
from political_profile.core.management.commands import treat_str, treat_date, fetch_url, DATE_FORMAT, perform_soap
...
class Command(BaseCommand):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.__map = self.__build_map()

    def handle(self, *args, **options):
        congressman_list = list(Congressman.objects.all())[::-1]
        # execute(congressman_list, self.__process)
        # self.__process(random.choice(congressman_list))

    def __process(self, congressman):
        self.stdout.write('{} - Listing votes.'.format(congressman.name))
        try:
            print(list(self.__process_votes_page(congressman)))
        except Exception as e:
            self.stderr.write(e)

    def __build_map(self):
        m = {}
        congressman = random.choice(list(Congressman.objects.all()))

        for *_, type_, number, year, vote in self.__process_votes_page(congressman):
            response = perform_soap(self.__client.service.ObterProposicao, **{
                'tipo': type_,
                'numero': number,
                'ano': year
            })
            proposition = lxml_to_dict(response)
            if 'proposicao' in proposition:
                proposition = proposition['proposicao']
            else:
                print(json.dumps(proposition, indent=2))
                continue
        return m
    ...
    def __process_votes_page(self, congressman):
        ...
        for d in votes:
            if d.has_attr('class'):
                ...
                type_, number, year = Command.__extract_proposition_metadata(proposition_link)

                yield (c_date, c_title, c_presence, c_reason, type_, number, year, vote)
